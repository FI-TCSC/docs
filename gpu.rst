...
Access
--------
Currently all GPU-nodes are on gpu-partition and access to resources is limited
to GPU usergroup. So if you are willing to use GPU's, you'll have to request
access to GPU-group via email to tcsc.tau@tuni.fi
...
This also shows the GPU utilization. Also seff-command is patched to include GPU utilization
off job::
    seff -j <jobID>
